...
**TRANCHE 2 IS PLANNED (25 Sep 2026) AND ITS CONTRACTS ARE `docs/prd/phase-14-tranche-2.md`** -- WP-14.16 through 14.32 on seven rulings, the first of which lifts tranche 1's freezes on MCP payloads, `rail.py`, `/api/health` and `core.overview()` PER ITEM AND NAMED: a package changes only the frozen surface its own item needs and says so. Read PRD §F (who owns which shared file) before touching one.
...
reads the JavaScript and holds all three against each other. Do not add a fourth copy.** Search is `⌘K` over `/api/search/index` (839 named things,
...
proportion packs; 1777 still wanted, and 858 of those can never be harvested) · 14 reference plans · 27 MCP tools · **54 checks, 2,746 tests**
(plus the workbench app suite, **515** under `node --test`; both measured 25 Sep 2026 at WP-14.15, the first as the sum of four `--shard i/4` runs, 24 failed + 2,697 passed + 25 skipped. The app suite had read 210 here since before Phase 12, while the suite grew past five hundred: nothing polices this line). Those figures were 970/36 before the
