...
**TRANCHE 2 IS BUILT, WP-14.16 THROUGH 14.32 (25–26 Sep 2026), ON SEVEN RULINGS, AND ITS CONTRACTS ARE `docs/prd/phase-14-tranche-2.md`.** The integration report is `docs/reports/wp-14.32-tranche-two-built.md`. What changed:
- Every slot, room, massing, grouping and parti has a page (`#/elements/<slot>`, `#/room/<id>`, …), and the "searched" banner is gone.
- Two styles compare at `#/compare/<a>/<b>`.
- A plan type starts a brief, and the composer guarantees it a place.
- The assistant is told the page's citation.
- Casings are drawn turned, and the Georgian wall prints 4 + 12 + 3.
- Twelve `--mark-*` tokens each carry one meaning, with a key.
- Every pick writes the URL, and no surface shows a default record.
- The reading surfaces reflow at 1280 px.
- The copy ratchet is a ban.
...
- A refusal is brick everywhere, and a link is ink with a gilt underline. A button that is a link takes `.tdl-link`, which carries the hover state an inline underline cannot.
- A corpus fact in app prose is derived or a glossary record. `copy_ratchet`'s PROSE scanner reads each JSX paragraph off the element tree, and STRINGS reads every string of twelve or more words. Both baseline by identity, with no `corpus` class.
- A description carries no build history: a parti's goes in its `note`, and `validate.py` sweeps the `description` of every record kind a page shows. Other fields those pages print are not swept: `oq/build-history-is-shown-in-page-fields-the-description-sweep-does-not-read`.
- A variant id repeats in a kit slot only as a condition, in the kit files (`check_kits.py::check_duplicate_variants`) and in the RESOLVED record, which is what consumers read.
...
- **And a pre-commit run piped through `tail` let red tests in**: `b33b210` went in with 2 failed, not 1. The second was `test_determinism.py::test_corpus_globs_are_sorted`, which stayed red until `36c52f9`. `51e5151` records the first. Read the exit code of pytest, not of the pipe.
- **A TEST THAT PATCHES AND THEN SOLVES POISONS THE SOLVE CACHE for every later call at the same key.** `geometry._SOLVE_CACHE` is keyed on nothing a test can patch. Two hearth tests solved the SHIPPED Tidewater record at the heuristic engine's key under a raising `stack_axes`, and the result outlived the patch. That put `test_openings.py`'s fixture pin red in one shard order, and it had kept `test_the_plate_draws_the_breast` green inside its own file for longer than this package. That test fails ALONE on WP-14.32's tree, 2 breasts against 3, and the plate is right, because the dining fire is refused and named. Pre-existing, and surfaced when two new test files repacked the shards. Use `monkeypatch.setattr(GEO, "_SOLVE_CACHE", {})` BEFORE the solve, or a clear in the `finally` of the `try` that solves. `tests/test_determinism.py` reads that off the AST now; its first version read text and passed an isolation made after the solve. **A test that passes only in its own file's order is a test of the order; run it alone before trusting it.**
- **THE ADVERSARIAL AUDIT OF WP-14.33 FOUND NOTHING BLOCKING AND A GREAT DEAL WORTH FIXING, ALL FIXED** (report §VI). The headline: **R8 was true of the kit files and false of the resolved kits.** `resolve_kit.apply_variant_ops` indexed an id to its LAST row, so a child's `replace` over a parent's plain-plus-conditional pair left the plain row standing, and `tidewater-georgian`'s gambrel resolved `permitted` AND `forbidden`. It acts on every inherited row of an id now: 6 triples over 3 styles went, and nothing else in the 164 moved. **And three copy scanners shared a regex comment-stripper that read the `accept=` file-type pattern in `Transcription.jsx` as a comment, so 79 of that file's lines were unread by every scanner in three test files.** `workbench/app/src/sourceReader.mjs` is the one lexer they share now, held against `@babel/parser` on every comment, JSX element, paragraph and string. **The rule it adds: a guard that holds TOKENS says nothing about the components that choose them.** Four refusals were drawn in `--sev-serious` or gilt while every token was right, and three navigating controls were still gilt.
...
reads the JavaScript and holds all three against each other. Do not add a fourth copy.** Search is `⌘K` over `/api/search/index` (887 named things,
