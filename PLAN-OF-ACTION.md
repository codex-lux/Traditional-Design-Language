...
6. **WP-4.6 — IN PROGRESS, nine packs of thirty-odd, 25 Aug 2026.** `moorish-arch` (OQ 30's item,
   ...
   with `trim-prairie`, which are both halves of PB-4's item, `dutch-gambrel`, `balcony-gallery` and `stone-course`. **All six packs this work package's own task
   ...
   **no opening-role pack 68 → 60**, which is where the movement now is, plus one wrong interior
   ...
   leverage, is in `docs/reports/wp-4.6-missing-proportion-packs.md`; next by the same measurement are a half-timber infill
   panel module (31 nodes) and a gable geometry system (12 nodes, entirely missing), after which the
   tail is mostly ORNAMENT rather than proportion. Then **WP-4.4** (images), which is currently
