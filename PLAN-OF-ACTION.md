...
6. **WP-4.6 — IN PROGRESS, eight packs of thirty-odd, 25 Aug 2026.** `moorish-arch` (OQ 30's item,
   ...
   with `trim-prairie`, which are both halves of PB-4's item, `dutch-gambrel` and `balcony-gallery`. **All six packs this work package's own task text names as
   likely candidates are now built.** 131 of 132 nodes bound; nodes with
   **no opening-role pack 68 → 60**, which is where the movement now is, plus one wrong interior
   ...
   leverage, is in `docs/reports/wp-4.6-missing-proportion-packs.md`; what remains is the long tail of WP-4.1's list,
   most of it ORNAMENT rather than proportion. Then **WP-4.4** (images), which is currently
...
**Status: IN PROGRESS, 8 of ~30 packs, 25 Aug 2026. All six packs this work package's own task text names as likely candidates are now built.** Chosen by measured leverage rather than by the order of the list; the measurement and what remains are in `docs/reports/wp-4.6-missing-proportion-packs.md`.
