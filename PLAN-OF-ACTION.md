...
**Status: THE PRECONDITION IS SETTLED (14 Sep 2026); THE RECORD EDIT IS NOT STARTED.** The span
half below shipped as its own commit — `geometry_cp._score` charges each room's own massing
element now — and the tagging has not been authored. This Status line exists because the
package was written without one, which §1's first rule forbids and which WP-5.3 and WP-5.4 had
their own Status lines written on 3 Sep to repair.
