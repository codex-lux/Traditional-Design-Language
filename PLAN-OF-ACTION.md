...
6. **WP-4.6 — IN PROGRESS, five packs of thirty-odd, 25 Aug 2026.** `moorish-arch` (OQ 30's item,
   ...
   written down in their own binding notes; bound to 9), `opening-pointed` and `opening-craftsman`
   (the opening halves of the Gothic and Craftsman items — both facade halves were measured and
   found already served). 131 of 132 nodes bound; nodes with **no opening-role pack 68 → 60**,
   which is where the movement now is. The remaining list, with each item's measured leverage, is in
   `docs/reports/wp-4.6-missing-proportion-packs.md`; the next by leverage is the Prairie trim
   family. Then **WP-4.4** (images), which is currently environment-blocked — see its own status
   block.
...
**Status: IN PROGRESS, 5 of ~30 packs, 25 Aug 2026.** Chosen by measured leverage rather than by the order of the list; the measurement and what remains are in `docs/reports/wp-4.6-missing-proportion-packs.md`.
