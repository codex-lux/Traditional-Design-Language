...
27 tools that let an AI consult the corpus while advising a human on a real house.
...
server cannot import. The decorator API is unchanged — all 27 tools are exactly as they
...
The same 27 tools are served at `/mcp` by the workbench, from the same process and the
