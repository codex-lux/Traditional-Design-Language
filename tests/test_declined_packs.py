...
def test_a_decline_counts_as_judged_and_not_as_unendorsed():
    ...
    assert len(declines) == 41, declines
...
def test_unendorsed_did_not_move_and_that_is_the_point():
    ...
    assert ci.RATCHET["unendorsed"] == 239
    assert ci.RATCHET_FLOOR["judged"] == 81
    assert ci.RATCHET["inherited_packs"] == 3325, (
        "forty-one declines removed forty-one real deliveries; 3366 was the figure before any")
