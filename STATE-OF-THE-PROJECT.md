...
**The proportion engine.** `selftest` resolves and dimensions all 36 packs with zero problems; `check_orders`, `check_modules` and `check_systems` pass with only explained warnings. Invariants are proved against the data, and the data encodes what the authorities *drew*, not what they said — Vignola's pedestal at 0.35 in Corinthian and Composite is the proof that the engine is honest.

**Proportion packs bound to the vocabulary — 129 of 132 buildable nodes** (was 5 at the last review). WP-4.1 bound every style and variant except three where no pack in the library honestly fits: `egyptian-revival` (trabeated, no arch), `moorish-andalusian` and `mudejar` (Islamic geometric setting-out). Those three sit in a named `DELIBERATELY_UNBOUND` allowlist in `check_pack_bindings.py --strict`, so the checker still catches a real regression while tolerating three checked exceptions. The gap is recorded as OQ 29 rather than gamed, and the ~25 missing packs the exercise identified feed WP-4.6.
