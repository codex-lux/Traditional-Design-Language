...
751 derived rules, **30 checks and 806 tests across 33 files** (was 22 and 419 across 22).
...
~~**Continuous integration.** The 304 tests exist; nothing runs them automatically.~~ *(Done — `.github/workflows/ci.yml` runs `check_all.py` and the workbench suite on every push; the count is 806 collected in `tests/` plus 91 in the workbench suite as of 26 Aug 2026 — the 762 stated here until then was stale by 33, counted before the 25 Aug merge.)*
...
| Checks | `tests/` | **806 tests, 33 files** | One base-branch failure (see PR #4); one run, ~8 min; CI on GitHub Actions |
