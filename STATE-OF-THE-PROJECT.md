...
| Governance | `docs/open-questions/` | **98 entries, 32 open** | **A DIRECTORY since 28 Aug 2026 (WP-8.1)** — one file per question, filename == id; `docs/open-questions.md` is a generated index. Four id-collision conversion tables are in its README: ids 32–41 → 54–63, two blocks of 64–66, 72–83 → 78–89, 78–85 → 91–98, plus the work-package renumber 5.7–5.10 → 5.11–5.14 (OQ 90). **No id may be renumbered again** — §1 now lists open questions and work packages as ids |
...
| Checks | `tests/` | **861 tests, 35 files** | One base-branch failure (see PR #4); one run, ~8 min; CI on GitHub Actions |
