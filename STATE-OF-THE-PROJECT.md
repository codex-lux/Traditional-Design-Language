...
751 derived rules, **30 checks and 762 tests across 32 files** (was 22 and 419 across 22).
...
~~**Continuous integration.** The 304 tests exist; nothing runs them automatically.~~ *(Done — `.github/workflows/ci.yml` runs `check_all.py` and the workbench suite on every push; the count is 762 collected in `tests/` plus 91 in the workbench suite as of 26 Aug 2026.)*
...
| Checks | `tests/` | **762 tests, 32 files** | One base-branch failure (see PR #4); one run, ~8 min; CI on GitHub Actions |
