...
**3. The vocabulary.** The style graph (`styles/`) — 164 taxa in four ranks (5 traditions, 27 families, 90 styles, 42 variants), spanning 700 BC to 2026, related by 476 typed lineage edges in a DAG rather than a tree. The single most important modelling decision in the whole project lives here: `descends_from` (actual transmission of practice) is a different edge from `references` (claimed ancestry), and only the former carries the kit-of-parts cascade. Greek Revival references Athens and descends from Federal carpentry; Colonial Revival references Georgian and descends from Beaux-Arts offices and the millwork catalog. A model that cannot express that gap will quietly produce wrong buildings. Each node also carries 624 massing affinities graded from `canonical` to `forbidden`, 660 enforceable constraints, 921 exemplars, and the binding of the node to its proportion packs.
...
**BUT READ OQ 51 BEFORE USING THIS NUMBER.** It counts a node's OWN `proportion_packs` array. `resolve_kit.resolve_packs` walks the entire `_cascade`, so a node also receives every pack its ancestors bound, judged for the ancestor and never for it. Measured: **222 (node, role) pairs** where an ancestor fills a role the node never bound, of which **180 involve a pack whose own `applies_to` does not name that node**; and **2,762** pack-arrivals purely by descent. (287/249/3,356 before WP-8.7's first adjudication pass on 2 Sep 2026, whose finding is that **this number refills as it is worked** — declining a pack re-attributes the role to the next ancestor, so fifteen declines moved it by four and one log node needs 26 declines to reach fixpoint. See `oq/a-node-that-refuses-a-category-must-decline-it-twenty-six-times`.) (This paragraph published 294/233/3,367 until 28 Aug 2026, when the WP-8.4 adversarial audit found it disagreeing with line 142 of this same file: WP-8.2 corrected the meter -- its ROLE loop lacked the `pack not in own_ids` guard its PACK loop had -- and both figures were wrong in the flattering direction.) `shotgun-house` takes its facade from `facade-peristyle` bound on `roman-classical`; `ranch-style` HAD 61 of 68 dimensioned slots governed by packs it never bound and now reads **59 of 66** — a fall that is the three opt-in flips REMOVING deliveries, not the node becoming better bound, and the same correction CLAUDE.md carries beside OQ 51's founding 69-of-78. `build/check_inheritance.py` reports it, names the ancestor that decided each one, and pins all three numbers. **RULED 25 Aug: adjudicate the 249 first, flip to opt-in inheritance after** — see Part III and Part V.
...
**Roof (WP-3.3).** Roof plan SVGs for both shipped plans, pitch inside the style band, chimneys satisfying the Tidewater constraint. *This sentence read "wing ridges stepping down" until 3 Sep 2026, and neither shipped plan carries the `dependency-and-hyphen` grouping that path needs — `TestWingStepDown::test_neither_shipped_plan_actually_carries_the_grouping` has pinned that as a fact, and `docs/structure.md` has said so, for as long as both have existed. The wing check is also **unjudged** now rather than passing: it derived the ridge from the very band it then tested, so it could not fail. See `docs/reports/project-review-2026-09-03.md` §IX.*
...
**132 of 132 buildable nodes**, with 423 surveys and 1,701 verbatim quotes. **WP-11.6 then executed
Ruling B**, giving all 27 families 121 type specimens DERIVED from their members' `standing: icon`
rows — one per member node, deduplicated by building, with a `why` that names the member and points
at that node for the reason rather than restating it. **915 of 921 exemplars carry a `precedent`,
across 159 nodes.** The 5 without one are the **traditions**, and the ruling says they stay empty, so
that figure may never reach 164 and that is the answer rather than a gap. `build/family_specimens.py`
re-derives and `check_precedents.py` fails the build when a family's rows drift from its members'. **What the literature then said about the kits is the
...
backlog; the other 38 were at least judged by somebody, and 208 have now been DECLINED. **3,341** pack-arrivals purely by descent. (The 294/233/61 first published, and the 293/222/71 after it, were both wrong in the flattering direction -- see WP-8.2.)
...
and it is the one with 2,762 instances. `build/check_inheritance.py --roles` and `--slots <node>`
...
**The image layer: 1850 records, 73 files, 858 naming a building** (revised 2 Sep 2026; it read "322 records, 0 files. Unchanged." for a week after none of those three numbers was true). The manifest was regenerated on 31 Aug from three style nodes to 142, and 73 records are `sourced` — every one of them a plate this corpus drew from its own proportion packs, none of them fetched. Every record carries a shot spec, alt text and provenance requirements. **858 name a real building to go and look for**, dealt from the depicted node's own `exemplars`, across 312 distinct queries of which 183 are inside HABS's US charter. Of the 1064 that name none, 858 are `role: incorrect` and never can (no archive indexes wrongness), 134 are drawings of a rule with nothing to photograph, and **72 are the live remainder** — photographs of styles whose node records no exemplars, all 18 of them higher-rank nodes. WP-4.4 wants ≥100 `sourced` or `generated` and zero `license: unknown`; it is at 73 and 1716, and neither can move without the network.
...
2. ~~**WP-4.4's offline half.** Giving the 322 asset records their `provenance.building` names needs no network and is the step the package itself names as next.~~ *(Done 31 Aug 2026: `build/name_asset_buildings.py` deals each node's records round its own exemplars, and 786 of 1850 records name a building across 305 queries. What is left offline is **72 records** whose style node records no exemplars — 18 higher-rank nodes — and that needs either exemplars authored from sources this container cannot reach or a ruling that a child's exemplar may stand for its parent. The package's acceptance line is unmet and cannot be met from here: it wants ≥100 sourced or generated against 73, and zero `license: unknown` against 1716.)*
...
| Evidence | `precedents/` | **695 records** after Tranche 3 and Ruling B (WP-11.2, WP-11.3, WP-11.5, WP-11.6; 159 of 164 nodes covered — every buildable node and every family, the 5 traditions empty by ruling) | WP-11.1 — the building behind an exemplar, refs with `retrieved`/`via`, the survey quoted, never a `license`; `check_precedents.py` |
