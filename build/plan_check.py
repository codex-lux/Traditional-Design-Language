...
def drawn_layer(plan, rooms, level_of, C, F):
    ...
    # level is generated by `slice_rect` with no reference to the ground placement, so the
    # search can only re-rank candidates that were produced blind. Raising the weight 100x
    # and 10,000x produced byte-identical output — the charge is a tax the search pays
    # forever and cannot act on. A hard CP constraint was refused for a different reason:
    # only `kind == "wall"` downgrades there, so a stacking constraint would be
    # non-downgradable and would outrank every declared exterior wall in the corpus —
    # measured, it silently downgraded an authored kitchen wall to satisfy an inferred
    # stack. Making the generator level-aware is the real fix and is its own package.
    out["stacks_broken"] = []
    for rid, r in rooms.items():
