...
"""
import json
import sys, os, glob, re, importlib.util, sys
...
def _states_assembly(pack_id, assembly_id):
    """Delegates to `proportion_engine.assembly_owner` -- one walk, in the engine, because five
    surfaces need the answer and a second hand-rolled copy had already appeared here."""
    return pe.assembly_owner(pack_id, assembly_id)


def _assembly_source(pack_id, assembly_id, resolved):
    """The citation for the members actually drawn, which is NOT always the pack's own.

    THE RECORD MUST NOT CONTRADICT THE PLATE IT POINTS AT. `palladio-tuscan` states no
    entablature on purpose -- Palladio does not dimension his Tuscan entablature and the plate
    numerals are illegible in every reachable scan, which is OQ 7 -- so the overlay hands it
    Vignola's cornice. This field used to read the RESOLVED pack's authority and so filed 34 of
    the 73 generated records under an authority that does not give the figures, on the four
    works OQ 7 through OQ 11 are about.

    The drawing was corrected first and this was not, which left the manifest asserting one
    thing and the SVG it names asserting another -- while the record's own alt_text closes
    "the drawing and the data cannot silently disagree". Fixing the ink and leaving the
    catalogue is the same defect one layer out."""
    owner = _states_assembly(pack_id, assembly_id)
    own = (resolved.get("authority") or {}).get("source")
    if not owner or owner == pack_id:
        return own
    src = (pe.PACKS.get(owner) or {}).get("authority") or {}
    return ("%s — INHERITED: %s states no %s of its own, and these members are %s's."
            % (src.get("source") or owner, pack_id, assembly_id, owner))
...
for pid in sorted(pe.PACKS):
    ...
    for asm in d["assemblies"]:
        ...
        assets.append({
            ...
            "provenance": {"source": _assembly_source(pid, asm["id"], pk), "license": "owned"},
...
#
# Rebuilding from scratch silently discarded: WP-4.4's hand-added `provenance.building` names
...
# by id.
# AN ALLOWLIST OF WHAT THIS SCRIPT OWNS, AND EVERYTHING ELSE IS CARRIED. Written the other way
# round at first -- a four-name list of fields to KEEP -- which is a denylist, and denies by
# default: the next top-level field any tool adds to a record would have been destroyed on the
# next regeneration while the run printed "carried forward N field(s)" as though nothing were
# lost. That is the identical shape to the country denylist corrected in harvest_habs.py in the
# same commit, and it was sitting in the same file with the allowlist already written above it
# and unused.
GENERATED_FIELDS = ("id", "kind", "role", "pair_with", "depicts", "caption", "alt_text",
                    "shot_spec", "priority", "tags", "generated_from")

prov_of = {a["id"]: (a.get("provenance") or {}).get("source")
           for a in assets if a.get("generated_from")}
...
for a in assets:
    ...
    for k, v in old_rec.items():
        if k in GENERATED_FIELDS or v in (None, {}, "", []):
            continue
        a[k] = v
        carried += 1

    # `depicts` IS generated -- except `faults`, which build/link_asset_faults.py derives and
    ...
    # PROVENANCE IS A MERGE, NOT A HANDOVER, and this is the one field where both sides own
    # something. What a harvester FOUND -- building, location, url, habs_number, rights_evidence,
    # a licence a person concluded -- is carried. But `source` on a record with `generated_from`
    # is DERIVED from the pack that states the assembly, and carrying the old one wholesale is
    # what kept 34 records citing Palladio for Vignola's cornice after the plate had been
    # corrected to say otherwise. The generator wins on that one key, and only there.
    if a.get("generated_from") and prov_of.get(a["id"]):
        a.setdefault("provenance", {})["source"] = prov_of[a["id"]]
...
manifest_io.write_manifest(manifest, "assets/manifest.json")
