...
**A fatal finding disqualifies a candidate**, and that is carried *beside* the score rather than inside it: `disqualified` is true and `disqualified_because` says so in words. A disqualified candidate never outranks a clean one whatever it scores — that guarantee lives in the sort's primary key, which is the **disqualifying** fatal count, so the score does not have to enforce it a second time.

**With one exception, ruled 20 September 2026 (WP-14.3): a fatal from an instrument the corpus has convicted REPORTS and does not disqualify.** The finding is emitted in full, at fatal severity, with its figure, and is still counted in `counts`; only `disqualified` and the sort's primary key stop reading it. The warrant is two-part and both halves are verified on every build: an entry in `critique/suspects.json` whose `basis` quotes where the generator admits the measurement is its own, **and** a `question` naming an OPEN register entry that names the fault. `fatal_excused`, `fatal_excused_rows` and `fatal_excused_because` ride on the candidate, and `fatal_excused_unjudged` is the third state — the sweep could not run, so nothing was excused. Measured over the sixteen shipped plans it excuses **one** finding of 158 and changes no plan's disqualification, because those houses' fatals are overwhelmingly the placement's; see `docs/reports/wp-14.3-a-convicted-instrument-reports.md`. In the workbench every ordering puts a disqualified candidate last, including "highest score first", and its column carries a band saying why.
