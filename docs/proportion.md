...
36 packs, in four kinds:

| Kind | Packs | Module |
|---|---|---|
| `order-system` | 5 Vignola orders, 19 authority overlays | the column semidiameter — or, for Palladio, the whole diameter |
| `module-system` | brick course, timber bay, sash light, storey graduation, log | a material unit |
| `facade-system` | classical, picturesque | the bay |
| `room-system` / `trim-system` / `opening-system` | harmonic, vernacular, classical trim, Craftsman trim, opening proportion | the room breadth, the ceiling, the door leaf |
