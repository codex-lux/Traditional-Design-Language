...
| `room-system` / `trim-system` / `opening-system` | harmonic, vernacular, classical trim, Craftsman trim, opening proportion, the pointed opening | the room breadth, the ceiling, the door leaf — for the pointed opening, the span, because a Gothic head's height is a consequence and not a dimension |
