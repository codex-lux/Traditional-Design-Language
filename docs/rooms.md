...
## Five things the catalogue insists on
...
**An aspect, and a machine-readable reading of it.** `daylight.orientation` says which way the room wants to face and why — the library's north because sun bleaches bindings, the kitchen's east because that is the hour it is worked in, the larder's north *"and it is not a preference"*. All sixty records carry one, and until WP-11.9 nothing in the tree read a single one. `daylight.aspect` is that reading in tokens, **authored beside the sentence and quoting it**, because four different jobs share one syntax in this prose: a governing aspect, an aspect conditioned on a climate the checker has no axis for, an axis of *doors* across the summer breeze, and a positive refusal (`garage`: *"Any"*). A pattern cannot tell those apart. Thirty-five records state an aspect, five of them hard; twenty-five answer with something that is not a compass and each says which, because **`applies: false` is a verdict and not a gap**. `build/check_rooms.py` holds every `basis` against the prose it claims to have read. `build/compass.py` is the one reader, and **plan-N is true-N unless `site.street_bearing_deg` says otherwise** — ruled 5 September 2026, with the cost that every finding prints the assumption it was judged under.
