...
No data file was touched. The four remaining unresolved slot-scattering and cascade-depth questions (14, 15, 18, 19, 21) were left exactly as open as they were found — this package only reconciles documentation against a schema/data reality that WP-1.1 and WP-1.3 (running concurrently) are actively changing, so it deliberately did not try to pre-narrate rulings that hadn't been made at the time this package ran.
