...
| R3 | The link ink reads below AA | **Ink text, gilt underline.** | `--link` is `--ink`, 9.48 : 1 on paper. The gilt is the underline: `--link-underline` was already gilt, and what is new is `--link-underline-hover` and the `.tdl-link` class that carries it to a button. `src/inks.test.mjs` computes the contrast on four papers and holds the tokens, the underline, every anchor and every clickable element's own ink (corrected by WP-14.33's audit, which found a navigating button with no underline invisible to the first two readers). |
