...
| `POST /api/compose` (`revise`, `revise_rounds` ≤ 8, `revise_engine`, `revise_budget_s` — the SET's budget, spent in rank order, cap 600; `candidates` ≤ 24) | WP-9.2: the returned candidates are placed and revised by default; the job streams a `revised` event per candidate with `score_before` / `score` and the drawn keys, or `revision_skipped` when the set's budget was spent before it. The bounds are `core`'s constants (one spelling with the MCP tools). 503 with `Retry-After` when `jobs.MAX_QUEUED` jobs are already waiting |
