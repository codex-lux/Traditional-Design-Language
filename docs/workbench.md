...
| `POST /api/scene` (`parti` as an ID; a record is 422; `plates`, default true; `candidates`) | WP-12.3: the constructed-3D record for the Round, with the PLACED plan and every named view's plate beside it — **one metered call and not seven**. `_heavy`, like every route that solves. Measured on `tidewater-georgian-careful`: a cold solve is 37.48 s, the six plates plus `build_scene` on the record it produces are 0.38 s, and the whole response is about 71 KB gzipped (376,574 raw) — so the budget rather than the clock bites, and at 60 calls an hour seven per record change buys eight edits where one buys sixty. Keep the returned `plan`: it carries `geometry`, so a later export or evaluate re-solves nothing. **It is not a sixth drawing kind** — that route returns an SVG and this returns a record. A plate that refuses is NAMED in `plates_refused`, never dropped |
