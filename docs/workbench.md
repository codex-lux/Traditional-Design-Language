...
| Route | What it adds |
|---|---|
| `GET /api/health` | asserts `jsonschema` importable (without it every check silently "fails schema"), reports counts and whether the rail has a key, and `session` (WP-14.20): whether THIS request would pass the gate, `auth.authorised`'s own answer, so a signed-out boot shows the Gate without asking anything gated |
...
| `POST /api/plan/evaluate` | solve, then `check()` on the SOLVED record (WP-9.1: the drawn layer runs and the elevation is of this placement — until then the declared record was checked and placed separately, and the drawn layer had never run on a bench evaluate), plus `plan` (WP-14.20: the id of the record evaluated, written before the early return a check that errored takes, so the journey can tell which house an errored evaluation was of), `fault_unjudged` (see below), `fault_not_applicable`, `rooms_meta` (per-room-type privacy rank, plumbing, daylight multiplier for the overlays) and `placement`, projected off the same solved object the sheet draws |
...
| `GET /api/glossary` · `GET /api/glossary/{id}` | WP-14.3: every definition the workbench shows, from `glossary/` (the app writes none) — the set's `version` (schema version + a digest of the records as authored), each record with `reads` (the files its basis quotes, by the pattern the checker verifies against), `by_family` and `by_field`; one term adds its confusables and names its `see` cites. **`/api/glossary/about-tdl` is the one ungated `/api/` path** besides health and login, ruled 24 Sep 2026 so the password screen can say what this is; `app.py`'s docstring says what it can carry. No MCP tool and no `overview()` key reads the glossary |
...
**Search.** `GET /api/search/index` serves every nameable thing once (814 entries, ~258 KB
served, measured 25 Sep 2026 after WP-14.17's glossary records; ~242 KB at 762 entries on the tree
...
as `short`, for display, and is never matched; indexing every fault's remedy and every style's
...
not name; `test_search_index.py` runs all 814 through the server's own validator.
