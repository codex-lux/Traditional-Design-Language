...
| `POST /api/compose` (`revise`, `revise_rounds`, `revise_engine`, `revise_budget_s`) | WP-9.2: the returned candidates are placed and revised by default; the job streams a `revised` event per candidate with `score_before` / `score` and the drawn keys |
